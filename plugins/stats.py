...
@bot_client.on(events.NewMessage(pattern=r"/transfer"))
async def transfer_premium_handler(event):
    ...
    if await is_premium_user(target_user_id):
        await event.respond("❌ The target user already has a premium subscription.")
        return
...
@bot_client.on(events.NewMessage(pattern=r"/rem"))
async def remove_premium_handler(event):
    user_id = event.sender_id
    if not await is_private_chat(event):
        return

    # OWNER_ID can be int, str, or list/tuple
    if isinstance(OWNER_ID, (list, tuple)):
