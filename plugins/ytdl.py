...
def progress_callback(done, total, user_id):
    ...
    completed_blocks = min(int(percent // 10), 10)
    ...
    remaining_time_min = remaining_time / 60 if remaining_time else 0
    ...
    final = (
        ...
        f"│ **__Speed:__** {speed_bps * 8 / (1024 * 1024):.2f} Mbps\n"
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    try:
        info_dict = await asyncio.to_thread(fetch_video_info, url, ydl_opts, progress_message, check_duration_and_size)
        if not info_dict:
            return
...
async def get_seconds(time_string: str) -> int:
    ...
    return value * time_units.get(unit.lower(), 0)
...
async def progress_bar(current: int, total: int, ud_type: str, message, start: float):
    ...
    if round(diff % 10) == 0 or current == total:
        ...
        completed_blocks = min(math.floor(percentage / 10), 10)
        progress = "".join(["♦" for _ in range(completed_blocks)]) + \
                   "".join(["◇" for _ in range(10 - completed_blocks)])
